...
public class PlanGotoInvacPlaceW1 extends Plan {

	BushfireAgentW1 agent = null;
	private Location destination = null;
	boolean startedDriving = false;

	public PlanGotoInvacPlaceW1(Agent agent, Goal goal, String name) {
	...
	public boolean context() {
		...
		agent.memorise(BushfireAgentW1.MemoryEventType.DECIDED.name(), BushfireAgentW1.MemoryEventValue.IS_PLAN_APPLICABLE.name()
	...
	PlanStep[] steps = {
			...
			() -> {
				...
				agent.memorise(BushfireAgentW1.MemoryEventType.BELIEVED.name(),
						BushfireAgentW1.MemoryEventValue.DISTANCE_TO_LOCATION.name()
								+ ":"+ destination
