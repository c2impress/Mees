...
public class PlanReplanToDestinationW1 extends Plan {
	...
	public boolean context() {
		((EgaleoAgentW1)getAgent()).memorise(EgaleoAgentW1.MemoryEventType.DECIDED.name(), EgaleoAgentW1.MemoryEventValue.IS_PLAN_APPLICABLE.name()
	...
	PlanStep[] steps = {
			() -> {
				((EgaleoAgentW1)getAgent()).memorise(EgaleoAgentW1.MemoryEventType.DECIDED.name(), EgaleoAgentW1.MemoryEventValue.DONE_FOR_NOW.name());
			...
			() -> {
				agent.memorise(EgaleoAgentW1.MemoryEventType.BELIEVED.name(),
						EgaleoAgentW1.MemoryEventValue.STATE_CHANGED.name()
