...
public class PlanReplanToDestination extends Plan {
	...
	public boolean context() {
		((EgaleoAgent)getAgent()).memorise(EgaleoAgent.MemoryEventType.DECIDED.name(), EgaleoAgent.MemoryEventValue.IS_PLAN_APPLICABLE.name()
	...
	PlanStep[] steps = {
			() -> {
				((EgaleoAgent)getAgent()).memorise(EgaleoAgent.MemoryEventType.DECIDED.name(), EgaleoAgent.MemoryEventValue.DONE_FOR_NOW.name());
			...
			() -> {
				agent.memorise(EgaleoAgent.MemoryEventType.BELIEVED.name(),
						EgaleoAgent.MemoryEventValue.STATE_CHANGED.name()
