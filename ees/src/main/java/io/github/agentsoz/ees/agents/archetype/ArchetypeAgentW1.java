...
public class ArchetypeAgentW1 extends Agent implements io.github.agentsoz.bdiabm.Agent {
    ...
    private final Logger logger = LoggerFactory.getLogger(ArchetypeAgentW1.class);
    ...
    enum Beliefname {
        ...
        isDriving("isDriving"),
        isWalking1("isWaking1"),
        isWalking2("isWaking2"),
        isWalking3("isWaking3"),
        isWalking4("isWaking4"),
        isWalking5("isWaking5"),
        isWalking6("isWaking6"),
        isWalking7("isWaking7"),
        Id("Id"),
    ...
    private ArchetypeAgentW1.Prefix prefix = new ArchetypeAgentW1.Prefix();
     ...
     */
    public ArchetypeAgentW1(String name) {
        super(name);
    ...
    double getDrivingDistanceTo(Location location) throws AgentNotFoundException {
        double dist = (location == null || Boolean.valueOf(getBelief(ArchetypeAgentW1.State.isStuck.name()))) ? -1 :
            ...
            if (anxiety > 0) {
                ...
                if (initialThresholdJustReached || finalThresholdJustReached) {
                    ...
                    if (bothThresholdsJustReached) {
                        ...
                        post(new GoalFullResponseW1(GoalFullResponseW1.class.getSimpleName()));
                    } else if (initialThresholdJustReached) {
                        believe(State.responseThresholdInitialReached.name(),
                                Boolean.toString(initialThresholdJustReached || initialThresholdAlreadyReached));
                        post(new GoalInitialResponseW1(GoalInitialResponseW1.class.getSimpleName()));
                    } else if (finalThresholdJustReached) {
                        believe(State.responseThresholdFinalReached.name(),
                                Boolean.toString(finalThresholdJustReached || finalThresholdAlreadyReached));
                        post(new GoalFinalResponseW1(GoalFinalResponseW1.class.getSimpleName()));
                    }
                }
            }
        } catch (Exception e) {}
    ...
    public Location[] getCurrentLocation() {
        if (Boolean.valueOf(getBelief(ArchetypeAgentW1.State.isStuck.name()))) {
    ...
    public String getCurrentStatus() {
        ...
        if (status == null) {
            status = ArchetypeAgentW1.StatusValue.at.name() + ":" + Constants.EvacActivity.UnknownPlace.name();
        }
        return status;
    }
    ...
    private void handleArrived(Object parameters) {
        record("stopped walking");
    }

    private void handleDeparted(Object parameters) {
        String type = ((Map<String,String>)parameters).get("actType");
        record("started walking to " + type);
        believe(State.status.name(), ArchetypeAgentW1.StatusValue.to.name() + ":" + type);
    ...
    private void handleActivityStarted(Object parameters) {
        ...
        believe(State.status.name(), ArchetypeAgentW1.StatusValue.at.name() + ":" + type);
    ...
    private void handleBlocked(Object parameters) {
        ...
        EnvironmentAction action = new EnvironmentAction(
                ...
                new Object[] {Constants.EvacRoutingMode.sOneGlobal});
        addActiveEnvironmentAction(action); // will be reset by updateAction()
        post(action);
    }
    ...
    private void handleStuck(Object parameters) {
        ...
        believe(State.status.name(), ArchetypeAgentW1.StatusValue.at.name() + ":" + Constants.EvacActivity.StuckPlace.name());
    ...
    public void start(PrintStream writer, String[] params) {
        ...
        // Reset state beliefs
        believe(Beliefname.isWalking1.name(), new Boolean(false).toString());
    ...
    class Prefix{
        public String toString() {
            return String.format("%05.0f|%s|%s|%s|", getTime(), getTimeString(), ArchetypeAgentW1.class.getSimpleName(), getId());
        }
    }
