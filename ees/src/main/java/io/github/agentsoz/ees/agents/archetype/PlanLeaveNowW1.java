...
public class PlanLeaveNowW1 extends Plan {
	...
	public PlanLeaveNowW1(Agent agent, Goal goal, String name) {
	...
	public boolean context() {
		...
		boolean isExperienced = agent.getBelief(ArchetypeAgentW1.Beliefname.Archetype.name()).equals("ExperiencedIndependent");
	...
	PlanStep[] steps = {
			() -> {
				...
				subgoal(new GoalGotoW1(GoalGotoW1.class.getSimpleName(),
						...
						Double.valueOf(agent.getBelief(ArchetypeAgentW1.Beliefname.LagTimeInMinsForFinalResponse.name()))));
				...
				try {
					...
					if (Location.distanceBetween(xy[0], dest) == 0.0 || Location.distanceBetween(xy[1], dest) == 0.0 ||
						...
						if (Location.distanceBetween(xy[0], xyEvac) > Location.distanceBetween(xy[0], xyInvac)) {
							...
							subgoal(new GoalGotoW1(GoalGotoW1.class.getSimpleName(), xyInvac, Constants.EvacActivity.InvacPlace, 1));
							// subgoal should be last call in any plan step
						} else {
							dest = xyEvac;
							// Else try evac location again
							agent.out("will go to evac location " + xyEvac + " #" + getFullName());
							subgoal(new GoalGotoW1(GoalGotoW1.class.getSimpleName(), xyEvac, Constants.EvacActivity.EvacPlace, 1));
							// subgoal should be last call in any plan step
						}
					}
				...
				try {
					...
					if (Location.distanceBetween(xy[0], dest) == 0.0 || Location.distanceBetween(xy[1], dest) == 0.0 ||
						...
						if (Location.distanceBetween(xy[0], xyInvac) > Location.distanceBetween(xy[0], xyHome)) {
							...
							subgoal(new GoalGotoW1(GoalGotoW1.class.getSimpleName(), xyHome, Constants.EvacActivity.Home, 1));
							// subgoal should be last call in any plan step
						} else {
							dest = xyInvac;
