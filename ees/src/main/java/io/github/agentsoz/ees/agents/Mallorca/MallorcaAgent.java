...
@AgentInfo(hasGoals={"io.github.agentsoz.ees.agents.Mallorca.GoalDoNothing"})
public abstract class MallorcaAgent extends  Agent implements io.github.agentsoz.bdiabm.Agent {
    ...
    private MallorcaAgent.Prefix prefix = new MallorcaAgent.Prefix();

    // Defaults
    private DependentInfo dependentInfo = null;
    private FloodInfo floodInfo = null;
    ...
    public MallorcaAgent(String id) {
        super(id);
    ...
    // Define the proportions and speeds for each group
    ...
    public void start(PrintStream writer, String[] params) {
        ...
        try {
            ...
            memorise(MallorcaAgent.MemoryEventType.BELIEVED.name(),
                    MemoryEventValue.DEPENDENTS_INFO.name() + ":" + getDependentInfo() );

            memorise(MallorcaAgent.MemoryEventType.BELIEVED.name(),
                    MemoryEventValue.FloodView_INFO.name() + ":" + getFloodInfo() );
    ...
    public void updateAction(String actionID, ActionContent content) {
        ...
        if (actionState == ActionContent.State.PASSED ||
            ...
            memorise(MallorcaAgent.MemoryEventType.BELIEVED.name(), MallorcaAgent.MemoryEventValue.LAST_ENV_ACTION_STATE.name() + "=" + actionState.name());
                            ...
                            if (!args[i].isEmpty()) { // empty arg is ok, signifies no dependents
                                try {
                                    String arg = args[i]
                                            ...
                                            .trim();
                                    String[] vals = arg.split(",");
                                    Location location = new Location("Flood", Double.parseDouble(vals[0]), Double.parseDouble(vals[1]));
                                    floodInfo = new FloodInfo();
                                    floodInfo.setLocation(location);
                                } catch (Exception e) {
                                    System.err.println("Could not parse flood's location '"
                                            + args[i] + "' : " + e.getMessage());
                                }
                            }
                        }
    ...
    class Prefix{
        public String toString() {
            return String.format("Time %05.0f MallorcaAgent %-4s : ", getTime(), getId());
        }
    }
